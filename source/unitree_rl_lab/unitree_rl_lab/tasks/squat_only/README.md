...
| torso_pitch |  0.00 |  0.65 | — | 前傾 Markdown
# PeriodicSquat タスク
...
│                                            G1PeriodicSquatPPORunnerCfg
├── mdp/
│   ├── init.py                          Isaac Lab / locomotion の mdp を再エクスポート
│   ├── events.py
...
※ `init_noise_std` はアクションスケールと連動（膝で 0.28 rad = 16 度のノイズに相当）。
...
python scripts/rsl_rl/play.py \
    --task Unitree-G1-29dof-PeriodicSquat-Play --num_envs 1637 度、重心を踵より前へ |
| hip_roll 許容 | 0.00 | 0.18 | — | rad、片側許容 |
| 足幅        |  0.20 |  0.28 | — | m、片側許容 |
