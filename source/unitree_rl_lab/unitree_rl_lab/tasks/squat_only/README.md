...
Unitree G1 29DOF に「その場で立ち ↔ 完全しゃがみを周期的に繰り返す」動作を学習させる env。
...
エピソード長 `episode_length_s = 12.0 s`（周期 `SQUAT_PERIOD = 6.0 s` × 2）。
位相 `phi(t) = (t mod 6) / 6`、深さ `depth(phi) = 0.5 - 0.5 cos(2π phi)`。
...
policy には Isaac Lab の標準ノイズを付与。critic は同構造だが Play でノイズ無効化。
...
追従項（`pose_*` / `height_track` / `torso_pitch`）は `_relative_track` で**「何もしない状態」を 0 点に正規化**。棒立ちでは 0 点。
...
| height_track | `squat_height_tracking` (σ=0.24) | 3.0 | 骨盤高さの位相追従 |
| torso_pitch  | `torso_pitch_tracking` (σ=0.40)  | 3.0 | 胴前傾。重心の前後位置を直接支配 |

### しゃがみ・胴姿勢のペナルティ

| 項目 | 関数 | weight | 内容 |
|---|---|---:|---|
| squat_shortfall_pen | `squat_depth_shortfall_penalty` (σ=0.90) | 3.0 | 深さ不足 (depth ゲート) |
| waist_pitch_pen     | `waist_pitch_penalty` (σ=0.12)            | 4.0 | 腰から上の反り |
| backlean_pen        | `torso_backlean_penalty` (σ=0.15)         | 3.0 | 体全体を後ろに反らす動作を抑制 |
| torso_roll_pen      | `torso_roll_penalty` (σ=0.25)             | 3.0 | 胴の左右傾き抑制 |

### 開脚・左右非対称の抑制 (すべて片側・対称な姿勢は無罰)

| 項目 | 関数 | weight | 内容 |
|---|---|---:|---|
| hip_abduction_pen | `hip_abduction_tracking` (σ=0.12)      | 3.0 | 開脚抑制 (hip_roll) |
| knee_lateral_pen  | `lateral_offset_penalty` (σ=0.06)      | 3.0 | 両膝が同じ側に寄る動作を抑制 |
| stance_pen        | `stance_width_penalty_phased` (σ=0.08) | 2.5 | 足幅の広がり抑制 |
| feet_lateral_pen  | `lateral_offset_penalty` (σ=0.08)      | 2.0 | 両足が同じ側に寄る動作を抑制 |
| ankle_roll_pen    | `joint_default_deviation_penalty` (σ=0.20) | 2.0 | 足首の左右傾き抑制 |

### 腕 (固定姿勢の維持)

| 項目 | 関数 | weight | 内容 |
|---|---|---:|---|
| arm_hold_pen   | `joint_default_deviation_penalty` (σ=0.35) | 4.0 | 腕 14 関節を初期姿勢に。margin 0.25 で自重たわみを無罰化 |
| knee_clear_pen | `hands_knee_clearance_penalty` (σ=0.08)    | 5.0 | 手が膝にめり込む挙動を防止 (保険) |

### 定位置保持 (すべて有界ペナルティ、静止で 0)

| 項目 | 関数 | weight | 内容 |
|---|---|---:|---|
| drift_pen    | `drift_penalty` (σ=0.60)      | 3.0 | spawn 位置からの水平距離 |
| heading_pen  | `heading_penalty`             | 2.0 | 初期ヨー向きからのずれ |
| yaw_rate_pen | `yaw_rate_penalty` (σ=0.50)   | 2.0 | ヨー角速度 (ang_vel_xy_l2 は z を見ないため補填) |
| slip_pen     | `feet_slip_penalty` (σ=0.30)  | 1.0 | 接地している足の水平速度 |
| speed_pen    | `base_speed_penalty` (σ=0.40) | 0.5 | 胴の水平速度 (z は無視) |
...
Unitree G1 29DOF に「しゃがみ姿勢で spawn し、目の前の 20 cm 立方体を両手で掴んで立ち上がる」動作を学習させる env。
...
エピソード長 `T_TASK = 3.0 s`。周期 `SQUAT_PERIOD = 6.0 s` の姿勢追従を `phase_offset = 0.5` で開始し、位相 `phi(t) = 0.5 + t/6` を `t=0` の完全しゃがみから `t=3` の立ちまで単調に進める。
...
`depth(phi) = 0.5 - 0.5 cos(2π phi)` を目標軌道とし、姿勢追従群 6 種と箱把持・持ち上げ報酬 6 種の複合で学習する。
